...
#define KEEPALIVE_INTERVAL                                                     \
	5000 // Marlin defaults to 2 seconds, get a little of margin
...
class GCodeDevice
    : public etl::observable< DeviceObserver, MAX_DEVICE_OBSERVERS > {
public:
	static GCodeDevice* getDevice ();
	// static void setDevice(GCodeDevice *dev);

	GCodeDevice (Stream* s, size_t priorityBufSize = 0, size_t bufSize = 0)
	    : printerSerial (s)
	    , connected (false)
	{
	...
	void addReceivedLineHandler (ReceivedLineHandler h)
	{
		receivedLineHandlers.push_back (h);
	}
...
protected:
	...
	char                curUnsentCmd[ MAX_GCODE_LINE + 1 ],
	    curUnsentPriorityCmd[ MAX_GCODE_LINE + 1 ];
	...
	void armRxTimeout ()
	{
		if (!canTimeout)
			return;
		// GD_DEBUGLN(enable ? "GCodeDevice::resetRxTimeout enable" :
...
public:
	MarlinDevice (Stream* s)
	    : GCodeDevice (s, 100, 200)
	{
		typeStr     = "marlin";
	...
	{
		GCodeDevice::begin ();
		if (!schedulePriorityCommand ("M115"))
			GD_DEBUGS ("could not schedule M115");
		if (!schedulePriorityCommand ("M114"))
			GD_DEBUGS ("could not schedule M114");
		if (!schedulePriorityCommand ("M105"))
			GD_DEBUGS ("could not schedule M105");
	}
	...
	const Temperature& getBedTemp () const
	{
		return bedTemperature;
	}
	const Temperature& getExtruderTemp (uint8_t e) const
	{
		return toolTemperatures[ e ];
	}
	uint8_t getExtruderCount () const
	{
		return fwExtruders;
	}
...
private:
	...
	static bool extractPrusaHeatingTemp (
	...
	static String
	    extractM115String (const String& response, const String field);

	static bool extractM115Bool (
	    const String& response,
	    const String  field,
	    const bool    onErrorValue = false);
};

String readStringUntil (Stream& PrinterSerial, char terminator, size_t timeout);
String
    readString (Stream& PrinterSerial, size_t timeout, size_t timeout2 = 100);
...
public:
	...
	static const uint32_t
	    serialBauds[]; // Marlin valid bauds (removed very low bauds; roughly
	                   // ordered by popularity to speed things up)

	static GCodeDevice* detectPrinter (HardwareSerial& PrinterSerial);

	static GCodeDevice* detectPrinterAttempt (
