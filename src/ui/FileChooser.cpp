...
void FileChooser::onButtonPressed (Button bt, int8_t arg)
{
	...
	case Button::ENC_DOWN:
		if (selLine < files.size () - 1)
		{
			...
			if (selLine >= topLine + VISIBLE_FILES)
			{
				topLine += VISIBLE_FILES - 1;
			}
	...
	case Button::BT1: {
		...
		{
			S_DEBUGF ("FileChooser::onButtonPressed(BT1): quit\n");
		...
		{
			S_DEBUGF (
			    "FileChooser::onButtonPressed(BT1): moving up from %s\n",
			    newPath.c_str ());
